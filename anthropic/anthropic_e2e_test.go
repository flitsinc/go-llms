...
func TestAnthropicE2E(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			maxTokens: 200,
			verifyRequest: func(t *testing.T, headers http.Header, body map[string]any) {
				assert.Nil(t, body["tools"], "Tools should not be present for JSON mode")
				assert.Nil(t, body["tool_choice"], "Tool choice should not be present for JSON mode")
				assert.NotNil(t, body["output_config"], "output_config should be present for JSON mode")
